...
load_sis_data <- function() {
  ...
  base_files <- list.files(path = sis_base_path, pattern = "data.*\\.xlsx", full.names = TRUE)
  ...
  sis_cases_path <- here::here("data", "raw", "service-system", "SiS_Power_BI", "Cases_Power_BI")
  cases_files <- list.files(path = sis_cases_path, pattern = "data.*\\.xlsx", full.names = TRUE)
